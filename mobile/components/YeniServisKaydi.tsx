...
// --- P2 DURUM PENCERESİ (KİBAR ANTRASİT) ---
...
export default function YeniServisKaydi({ visible, onClose }: any) {
  ...
  // MECBURİ GİRİŞ DENETİMİ
  const handleSaveAttempt = () => {
    if (servis.cihaz_sahibi.trim().length < 2) {
      setStatus({ visible: true, type: 'error', msg: 'Müşteri ismi girmeden kayıt yapılamaz.' });
      return;
    }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.fullContainer}>
        <SafeAreaView style={styles.safeArea}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} bounces={false} keyboardShouldPersistTaps="always">
            ...
            <View style={[styles.row, focusField === 'sahibi' && styles.focusedBorder]}>
              <TextInput ref={r1} style={[styles.input, {flex: 1, borderWidth: 0}]} placeholder="İsim veya Tel..." 
                onFocus={() => setFocusField('sahibi')}
                onChangeText={(v)=>setServis({...servis, cihaz_sahibi: v})} 
                returnKeyType="next" onSubmitEditing={() => { Keyboard.dismiss(); setModalType('tür'); }} />
              <TouchableOpacity style={styles.searchBtn}><Ionicons name="search" size={24} color="#fff" /></TouchableOpacity>
            </View>

            {/* CİHAZ TÜRÜ (BINME SORUNU GIDERILDI) */}
            ...
            <TextInput ref={r6} style={[styles.input, focusField === 'aksesuar' && styles.focusedBorder]} 
              onFocus={() => setFocusField('aksesuar')}
              placeholder="Çizik, adaptör vb."
          ...
          <CustomSelect visible={modalType === 'tür'} title="CİHAZ TÜRÜ" data={['Cep Telefonu', 'Masaüstü Bilgisayar', 'Notebook', 'Yazıcı', 'TV', 'Tablet']} onSelect={(v: string) => { setServis({...servis, cihaz_turu: v}); setModalType(null); setFocusField('marka'); setTimeout(() => r2.current?.focus(), 400); }} onClose={() => setModalType(null)} />
          <CustomSelect visible={modalType === 'garanti'} title="GARANTİ" data={['Yok', 'Var (Resmi)', 'Var (Dükkan)']} onSelect={(v: string) => { setServis({...servis, garanti: v}); setModalType(null); setFocusField('not'); setTimeout(() => r5.current?.focus(), 400); }} onClose={() => setModalType(null)} />
          ...
          <StatusModal visible={status.visible} type={status.type} message={status.msg} onConfirm={() => { setStatus({ ...status, visible: false }); if (status.type === 'success') onClose(); }} />
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: '#f2f2f2', borderWidth: 1, borderColor: '#eee', borderRadius: 12, padding: 12, fontSize: 15, color: '#000', marginBottom: 5 },
  focusedBorder: { borderColor: '#FF3B30', borderWidth: 1.5, backgroundColor: '#fff' }, // TAKIP CIZGISI
  p2SelectBox: { backgroundColor: '#f2f2f2', borderRadius: 12, padding: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderWidth: 1, borderColor: '#eee', marginBottom: 5 },
  p2SelectText: { fontSize: 15, color: '#000', fontWeight: '500' },
  row: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f2f2f2', borderRadius: 12, borderWidth: 1, borderColor: '#eee', marginBottom: 5 },
  rowLayout: { flexDirection: 'row', justifyContent: 'space-between' },
  searchBtn: { backgroundColor: '#1A1A1A', width: 48, height: 48, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
