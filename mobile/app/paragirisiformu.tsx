...
export default function ParaGirisiFormuV2() {
  ...
  /**
   * TAHSİLAT KAYIT VE VERİTABANI YAZMA (BUDANMADI)
   */
  const handleFinalTahsilat = async () => {
    ...
    try {
      ...
      const resData = await response.json();
      if (resData.success) {
        Alert.alert("İŞLEM BAŞARILI", "Kasa hareketi kaydedildi.", [
...
const styles = StyleSheet.create({
  ...
  input: { padding: 16, borderRadius: 15, fontSize: 18 },
  staticInput: { padding: 16, borderRadius: 15, marginBottom: 15 },
  searchContainer: { marginBottom: 15 },
  searchRow: { flexDirection: 'row', gap: 10 },
  searchBtn: { width: 60, borderRadius: 15, justifyContent: 'center', alignItems: 'center' },
  infoList: { padding: 20, borderRadius: 20, marginTop: 10, borderWidth: 1.5, borderStyle: 'solid' },
