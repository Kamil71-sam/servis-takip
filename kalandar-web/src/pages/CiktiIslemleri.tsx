...
export default function CiktiIslemleri({ defaultTab = 'fatura' }: { defaultTab?: 'fatura' | 'servis' | 'tamamlanan' }) {
  ...
  // 🚨 MÜDÜRÜN YEDEK PARÇA HAFIZASI (YENİ EKLENDİ)
  ...
  // 🚨 YEDEK PARÇA ÇEKİCİ MOTORU (YENİ EKLENDİ)
  useEffect(() => {
  ...
  // 🚨 SENİN TERTEMİZ PDF MOTORUN (HİÇ DOKUNULMADI)
  ...
  // 🚨 SENİN TERTEMİZ SANAL YAZDIRMA (HIZLI YAZDIR) MOTORUN (HİÇ DOKUNULMADI)
  const handlePrint = () => {
    if (!seciliKayit) return;
  ...
  return (
    <>
      ...
      {/* ================= 🚨 HAYALET A4 MATBAASI (PDF ve YAZICI İÇİN) 🚨 ================= */}
      <div className="absolute z-[-50] opacity-0 pointer-events-none left-0 top-0">
        {seciliKayit && (
          
          <div id="ghost-print-area" style={{ 
            width: '794px', 
            padding: '20px', /* 🚨 Boşluklar PDF motorundan (margin:10) geleceği için abartı padding silindi */
            ...
            <table style={{ width: '100%', borderBottom: '4px solid #a61c24', marginBottom: '25px', paddingBottom: '15px' }}>
              <tbody>
                <tr>
                  {/* SOL TARAF: FİRMA ADI (%50 ALAN KİLİDİ) */}
                  <td style={{ verticalAlign: 'top', width: '50%', paddingRight: '10px' }}>
                    <h1 style={{ fontSize: '46px', fontWeight: '900', margin: 0, letterSpacing: '-1px', textTransform: 'uppercase' }}>
                  ...
                  {/* SAĞ TARAF: BELGE TÜRÜ VE TARİH (%50 ALAN KİLİDİ) */}
                  <td style={{ textAlign: 'right', verticalAlign: 'top', width: '50%' }}>
                    <h2 style={{ fontSize: '22px', fontWeight: '900', color: '#a61c24', margin: '0 0 10px 0', textTransform: 'uppercase', letterSpacing: '1px' }}>
              ...
              <tbody>
                ...
                {aktifKayit.tip === 'Servis' && servisParcalari.map((parca, idx) => (
                  <tr key={idx} style={{ pageBreakInside: 'avoid' }}>
                    <td style={{ textAlign: 'center', verticalAlign: 'top', padding: '15px 10px', border: '1px solid #eee', color: '#666', fontWeight: 'bold', fontSize: '13px' }}>{idx + 1}</td>
                    ...
                    <td style={{ padding: '15px 10px', border: '1px solid #eee', textAlign: 'center', verticalAlign: 'top', fontWeight: 'bold', fontSize: '13px' }}>{parca.quantity || parca.miktar || 1}</td>
                    <td style={{ padding: '15px 10px', border: '1px solid #eee', textAlign: 'right', verticalAlign: 'top', fontWeight: 'bold', fontFamily: 'monospace', fontSize: '14px' }}>0,00 ₺</td>
                ...
                <tr style={{ pageBreakInside: 'avoid' }}>
                  ...
                  <td style={{ padding: '15px 10px', border: '1px solid #eee', verticalAlign: 'top', fontWeight: 'bold', fontSize: '13px' }}>
                    <div style={{ whiteSpace: 'pre-line', lineHeight: '1.6' }}>{aktifKayit.tip === 'Servis' ? 'İşçilik ve Genel Servis Bedeli' : aktifKayit.aciklama}</div>
                    {aktifKayit.tip !== 'Stok Satışı' && (
                        <div style={{ fontSize: '11px', color: '#888', marginTop: '6px' }}>(Kayıt No: {aktifKayit.servis_no}{aktifKayit.raw?.barkod ? `, Barkod: ${aktifKayit.raw.barkod}` : ''})</div>
                    )}
                    {aktifKayit.tip === 'Servis' && aktifKayit.aciklama && (
                        <div style={{ fontSize: '11px', color: '#888', marginTop: '2px' }}>Arıza/Not: {aktifKayit.aciklama}</div>
                    )}
                  </td>
      ...
      {/* 🚨 ANA EKRAN KİLİDİ: Tüm sayfanın kaymasını yasakladık, ekrana çiviledik 🚨 */}
      <div className="flex flex-col h-[calc(100vh-40px)] overflow-hidden p-1 gap-3">
          ...
          <div className="w-[400px] flex flex-col bg-[#1A1A1E] border border-white/5 rounded-2xl overflow-hidden shadow-2xl shrink-0 relative z-10">
            ...
            <div className="flex-1 overflow-y-auto nuke-scrollbar p-2.5 flex flex-col gap-2">
              {loading ? <div className="text-center py-10 text-gray-600 text-[11px] font-black uppercase animate-pulse">Yükleniyor...</div> : 
               ...
               currentRecords.map((item) => {
                let cardColorClass = '';
                if (item.tip === 'Servis') cardColorClass = seciliKayit?.id === item.id ? 'bg-orange-500/30 border-orange-500' : 'bg-orange-500/10 border-orange-500/20 hover:bg-orange-500/20';
                else if (item.tip === 'Randevu') cardColorClass = seciliKayit?.id === item.id ? 'bg-blue-500/30 border-blue-500' : 'bg-blue-500/10 border-blue-500/20 hover:bg-blue-500/20';
                else if (item.tip === 'Stok Satışı') cardColorClass = seciliKayit?.id === item.id ? 'bg-green-500/30 border-green-500' : 'bg-green-500/10 border-green-500/20 hover:bg-green-500/20';
                else cardColorClass = seciliKayit?.id === item.id ? 'bg-[#8E052C]/30 border-[#8E052C]' : 'bg-white/5 border-white/5 hover:bg-white/10';
            ...
            <div className="p-3 border-t border-white/5 bg-black/40 flex justify-center items-center gap-1.5">
              ...
              {getPageNumbers().map((p, i) => (
                p === '...' ? <span key={`dots-${i}`} className="text-gray-500 text-[10px] px-1 font-black">...</span> : 
