      ...
      <div className="flex flex-col h-full w-full max-w-4xl mx-auto p-4 animate-in fade-in slide-in-from-bottom-4">
        ...
        <div className="bg-[#1A1A1E] border border-white/5 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
          ...
          <div className="p-8 grid grid-cols-2 gap-8 overflow-y-auto nuke-scrollbar">
            ...
            <div className="col-span-2 mt-2 pt-6 border-t border-white/5">
                <h3 className="text-[#8E052C] text-sm font-black uppercase tracking-widest flex items-center gap-2 mb-4">
                  <span className="text-xl">👨‍💻</span> Sistem Yöneticisi Bilgileri
                </h3>
                <p className="text-[10px] text-gray-500 mb-4 uppercase font-bold tracking-widest">Sol menünün alt kısmında görünecek olan profil isim ve unvanınız.</p>
                
                <div className="grid grid-cols-2 gap-8">
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Adınız Soyadınız</label>
                        <input type="text" value={ayarlar.mudur_adi || ''} onChange={(e) => handleDegeisim('mudur_adi', e.target.value)} placeholder="Örn: KEMAL MÜDÜR" className="w-full bg-black/50 border border-[#8E052C]/30 focus:border-[#8E052C] rounded-xl px-4 py-3 text-sm text-white outline-none font-bold transition-all shadow-inner" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Unvanınız</label>
                        <input type="text" value={ayarlar.mudur_unvani || ''} onChange={(e) => handleDegeisim('mudur_unvani', e.target.value)} placeholder="Örn: PATRON" className="w-full bg-black/50 border border-[#8E052C]/30 focus:border-[#8E052C] rounded-xl px-4 py-3 text-sm text-white outline-none font-bold transition-all shadow-inner" />
                    </div>
                </div>
            </div>
            ...
            <div className="col-span-2 flex flex-col gap-1.5 mt-2 border-t border-white/5 pt-6">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Matbu Evrak Alt Bilgi / Yasal Uyarı</label>
              <p className="text-[9px] text-gray-500 mb-1 uppercase font-bold tracking-widest">Cihaz teslim fişi ve faturaların en altında görünecek olan yasal uyarı veya teşekkür metni.</p>
