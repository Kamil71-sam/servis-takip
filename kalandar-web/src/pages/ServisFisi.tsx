...
// 🚨 MÜDÜR: Her türlü veri tipini (Türkçe/İngilizce/Büyük/Küçük) yakalayan zırhlı mıknatıs
const getVal = (obj: any, keys: string[]) => {
  ...
  for (let k of keys) {
    ...
    if (lowerObj[key] !== undefined && lowerObj[key] !== null && lowerObj[key] !== '') {
      return String(lowerObj[key]);
    }
  }
  return '';
};

// 🚨 MÜDÜR: Noktalı tarihleri tireye çevirip "Invalid Date" hatasını bitiren motor
const generateDateStamp = (rawDate?: any) => {
  if (!rawDate) return '------';
  let safeDate = String(rawDate);
  if (safeDate.includes('.')) {
    // 19.04.2026 -> 2026-04-19 (JS'nin anladığı dil)
    const parts = safeDate.split(' ')[0].split('.');
...
export default function ServisFisi() {
  ...
  useEffect(() => {
    setSeciliKayit((prevSecili) => {
      ...
      if (aramaTarih && prevSecili.tarih) {
        let sd = String(prevSecili.tarih);
        if(sd.includes('.')) { 
           const p = sd.split(' ')[0].split('.'); 
           if(p.length===3) sd = `${p[2]}-${p[1]}-${p[0]}`; 
        }
        const d = new Date(sd);
        if (!isNaN(d.getTime())) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 1. AKTİF SERVİSLERİ ÇEK VE SADECE YENİ KAYITLARI/PARÇA BEKLEYENLERİ AL
      ...
      const yeniKayıtServisler = servisAktifListe
        .filter((i: any) => (i.status === 'Yeni Kayıt' || i.durum === 'Yeni Kayıt' || i.status === 'Parça Bekliyor'))
        .map((i: any) => ({
            id: `srv_${i.id}`, tip: 'Servis', 
            servis_no: i.plaka || i.servis_no || 'Bilinmiyor',
            musteri_adi: getVal(i, ['musteri_adi', 'name', 'firma_adi', 'musteri', 'müşteri adı']) || 'Bilinmiyor', 
            tarih: i.tarih || i.created_at || i.updated_at, 
            tutar: i.offer_price || i.fiyat || 0, 
            durum: i.status || i.durum, 
            cihaz_bilgisi: i.marka_model || `${i.cihaz_tipi || ''} ${i.brand || i.marka || ''} ${i.model || ''}`.trim() || 'Cihaz Belirtilmemiş', 
      ...
      uniqueData.sort((a, b) => {
          // Tarihleri sıralama için güvenli hale getir
          let dateA = a.tarih;
          let dateB = b.tarih;
          if (typeof dateA === 'string' && dateA.includes('.')) { const p = dateA.split(' ')[0].split('.'); dateA = `${p[2]}-${p[1]}-${p[0]}`; }
          if (typeof dateB === 'string' && dateB.includes('.')) { const p = dateB.split(' ')[0].split('.'); dateB = `${p[2]}-${p[1]}-${p[0]}`; }
  ...
  const renderA5Nusha = (nushaTipi: string, isTop: boolean) => {
    const sTarih = seciliKayit?.tarih || '';
    let formatliTarih = '----/----/----';
    if(sTarih) {
        let sd = String(sTarih);
        if(sd.includes('.')) { 
            const p = sd.split(' ')[0].split('.'); 
            if(p.length===3) sd = `${p[2]}-${p[1]}-${p[0]}`; 
        }
        const d = new Date(sd);
        if(!isNaN(d.getTime())) formatliTarih = d.toLocaleDateString('tr-TR');
    }
                ...
                <table style={{ width: '180px', marginLeft: 'auto', fontSize: '11px', fontWeight: 'bold' }}>
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td style={{ textAlign: 'right', fontFamily: 'monospace' }}>{seciliKayit ? formatliTarih : '----/----/----'}</td>
                    </tr>
                  </tbody>
                </table>
              </td>
            ...
            <tr>
              ...
              <td style={{ width: '33.33%', verticalAlign: 'top', paddingRight: '15px' }}>
                  ...
                  <div style={{ fontWeight: 'bold', lineHeight: '1.6', minHeight: '40px' }}>
                      ...
                      Adres: {getVal(gosterilecekData.raw, ['customer_address', 'adres', 'address']) || '-'}<br />
                      GSM: {getVal(gosterilecekData.raw, ['customer_phone', 'telefon', 'phone', 'gsm']) || '-'}
                  </div>
              </td>
              ...
              <td style={{ width: '33.33%', verticalAlign: 'top' }}>
                ...
                <div style={{ fontWeight: 'bold', lineHeight: '1.6', minHeight: '40px' }}>
                  ...
                  {/* 🚨 MÜDÜR: İŞTE BURASI! Seri numarası değil, direkt senin kendi takip numaran (servis_no) basılıyor */}
                  Seri/Barkod: {seciliKayit ? (gosterilecekData.servis_no || '-') : ''}
                </div>
              </td>
            </tr>
          </tbody>
            ...
            <div className="flex-1 overflow-y-auto nuke-scrollbar p-2.5 flex flex-col gap-2">
              {loading ? <div className="text-center py-10 text-gray-600 text-[11px] font-black uppercase animate-pulse">Yükleniyor...</div> : 
               ...
               currentRecords.map((item) => {
                ...
                // MÜDÜR: Sol listedeki tarih görünümünü de zırhladık
                let solListeTarih = '';
                if(item.tarih) {
                   let sld = String(item.tarih);
                   if(sld.includes('.')) { const p = sld.split(' ')[0].split('.'); if(p.length===3) sld = `${p[2]}-${p[1]}-${p[0]}`; }
                   const d = new Date(sld);
                   if(!isNaN(d.getTime())) solListeTarih = d.toLocaleDateString('tr-TR');
                }
                ...
                return (
                  <div key={item.id} onClick={() => setSeciliKayit(item)} className={`p-3 rounded-xl border cursor-pointer transition-all ${isSelected ? `${cardColorClass}/30 border-${cardColorClass.split('-')[1]}-500` : `${cardColorClass}/10 border-${cardColorClass.split('-')[1]}-500/20 hover:${cardColorClass}/20`}`}>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-[9px] text-gray-400 font-mono font-bold mt-1">{solListeTarih}</span>
                    </div>
