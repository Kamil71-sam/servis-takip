      ...
      <div className="w-full min-w-[1100px] min-h-screen flex transition-all duration-500">
        ...
        <div className="w-[280px] border-r border-white/5 bg-black/40 backdrop-blur-md p-6 flex flex-col shrink-0 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-3 overflow-y-auto pr-2 scrollbar-hide relative z-10 pb-4">
            ...
            {[
              ...
              //{ isim: 'Mali İşlemler', ikon: '💳', altMenuler: ['Mali Durum', 'Tahsilatlar', 'Cari Hesaplar'] },
                ...
                <button 
                  ...
                  <div className="flex flex-col gap-1 pl-11 pr-2 border-l-2 border-[#8E052C]/20 ml-5">
                    {menu.altMenuler.map((altItem, i) => {
                      ...
                      // 🚨 MÜDÜR: TÜRKÇE KARAKTER KRİZİNİ ÇÖZEN AKILLI MOTOR 🚨
                      let hedefSayfa = altItem.replace(/ /g, '');
                      if (altItem === 'Bireysel Müşteri Kaydı') hedefSayfa = 'BireyselMusteriKaydi';
                      if (altItem === 'Firma Kaydı') hedefSayfa = 'FirmaKaydi';
                      if (altItem === 'Müşteri Listesi') hedefSayfa = 'MusteriListesi';
                      if (altItem === 'Yeni Servis Kaydı') hedefSayfa = 'YeniServisKaydi';
                      if (altItem === 'Servis Kayıtları') hedefSayfa = 'ServisKayitlari';
                      if (altItem === 'Tamamlanan İşler') hedefSayfa = 'TamamlananIsler';
                      if (altItem === 'Yeni Randevu') hedefSayfa = 'YeniRandevu';
                      if (altItem === 'Randevu Takvimi' || altItem === 'Randevu Kayıtları') hedefSayfa = 'RandevuTakvimi';
                      if (altItem === 'Tamamlanan Randevular') hedefSayfa = 'TamamlananRandevular';
                      if (altItem === 'Stok Durumu') hedefSayfa = 'StokDurumu';
                      if (altItem === 'Malzeme Girişi') hedefSayfa = 'MalzemeGirisi';
                      if (altItem === 'Yedek Parça Takibi') hedefSayfa = 'YedekParcaTakibi';
                      ...
                      return (
                        <button 
                          key={i} 
                          onClick={() => setAktifSayfa(hedefSayfa)}
                          ...
                          <div className={`w-1.5 h-1.5 rounded-full transition-colors ${
                            isActive ? 'bg-[#8E052C]' : 'bg-gray-700 group-hover:bg-[#8E052C]'
                          }`}></div>
                          {altItem}
                        </button>
                      )
        ...
        <div className="flex-1 p-8 flex flex-col gap-6 relative min-w-0">
          ...
          <div className="flex justify-between items-center z-10 border-b border-white/5 pb-4 mb-2">
            <div>
              {/* 🚨 ZİNCİRLERİ DÜZELTİLMİŞ BAŞLIK (H1) 🚨 */}
              ...
              {/* 🚨 ZİNCİRLERİ DÜZELTİLMİŞ AÇIKLAMA (P) 🚨 */}
              <p className="text-gray-500 text-xs mt-1.5 font-black uppercase tracking-widest">
                ...
                aktifSayfa === 'KasaCikisi' ? 'Kasadan manuel masraf ve gider çıkış işlemleri.' : 
                'Kayıt Takip Listesi'}
                </p>
          ...
          {aktifSayfa === 'Dashboard' ? (
              ...
              <>
                ...
                <div className="grid grid-cols-5 gap-6 flex-1 min-h-[350px]">
                  ...
                  <div className="col-span-2 bg-black/40 border border-white/5 p-8 rounded-[2.5rem] flex flex-col relative overflow-hidden shadow-2xl">
                     ...
                     <div className="flex-1 flex flex-col items-center justify-center z-10 gap-8 mt-2">
                        <div 
                          className="w-48 h-48 rounded-full relative flex items-center justify-center shadow-[0_0_30px_rgba(0,0,0,0.5)] shrink-0"
                          style={{ background: `conic-gradient(#8E052C 0% ${randevuYuzde}%, #27272A ${randevuYuzde}% 100%)` }}
                        >
                           <div className="absolute inset-[15px] bg-[#0F0F12] rounded-full border border-white/5 flex flex-col items-center justify-center shadow-inner">
                              <div className="text-5xl font-black text-white leading-none tracking-tighter">{toplamAktif}</div>
                              <div className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1">Aktif İşlem</div>
                           </div>
                        </div>

                        <div className="flex items-center justify-center gap-6 w-full">
                           <div className="flex items-center gap-2.5">
                             <div className="w-3.5 h-3.5 rounded-full bg-[#8E052C] shadow-[0_0_10px_#8E052C]"></div>
                             <span className="text-xs text-gray-300 font-black uppercase tracking-widest">{dashboardData.aktif_randevu} Randevu</span>
                           </div>
                           <div className="flex items-center gap-2.5">
                             <div className="w-3.5 h-3.5 rounded-full bg-[#27272A] border border-gray-600"></div>
                             <span className="text-xs text-gray-300 font-black uppercase tracking-widest">{dashboardData.aktif_servis} Servis</span>
                           </div>
                        </div>
                     </div>
                  </div>
            ...
            <YedekParcaTakibi />  
          ) : aktifSayfa === 'MaliDurum' ? ( 
            <MaliDurum onSeeAll={() => setAktifSayfa('TumMaliHareketler')} />
          ) : aktifSayfa === 'TumMaliHareketler' ? ( 
